    ...
    if request.method == 'GET':
        try:
            if request.GET['character']:
                character_id = request.GET['character']
                try:
...
#PUT
    if request.method == 'PUT':
        ...
        try:
            ...
            update_character = Character.objects.get(pk=character_id)
        except:
            return Response(status=status.HTTP_404_NOT_FOUND)
        print(request.data)
...
#DELETE
    if request.method == 'DELETE':
        try:
            character_to_delete = Character.objects.get(pk=request.data[character_id])
